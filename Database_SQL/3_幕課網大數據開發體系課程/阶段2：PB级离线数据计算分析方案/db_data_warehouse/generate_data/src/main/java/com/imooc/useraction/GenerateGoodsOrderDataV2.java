...
/**
 * 需求：生成商品订单相关数据(服务端数据)
 * 生成2026-02-01~2026-02-28的数据
 * Created by xuwei
 */
public class GenerateGoodsOrderDataV2 {
    private final static Logger logger = LoggerFactory.getLogger(GenerateGoodsOrderDataV2.class);

    public static void main(String[] args) throws Exception{
        //通过接口获取用户行为数据
        String dataUrl = "http://data.xuwei.tech/d1/go2";
        ...
        if(!flag){
            ...
            String[] dateArr = GenerateDateAndActArrUtil.getDateArr();
            String[] tableArr = GenerateDateAndActArrUtil.getTableArr();
            for(int i=0;i<dateArr.length;i++){
                String dt = dateArr[i];
                for(int j=0;j<tableArr.length;j++){
                    String tableName = tableArr[j];
                    //获取某一天某一类型的数据
